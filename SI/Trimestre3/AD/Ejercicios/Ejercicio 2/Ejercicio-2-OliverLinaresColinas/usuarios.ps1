...
#Comprobación de argumentos y del fichero csv
if($args.Count -lt 2){
    ...
    if(-Not (Get-ADGroup -Filter "Name -eq '$($args[0])'")){
    ...
    if(-Not (Get-ADOrganizationalUnit -Filter "Name -eq '$($args[1])'")){
    ...
    foreach($u in $usuarios){
        ...
        if(Get-ADUser -Filter "Name -eq '$($u.nombre)'"){
            ...
            Write-Host "Usuario $($u.nombre) creado correctamente"
        }
    }
}